...
<tr>
  <td>08.30 - 09.00</td>
  <td style="vertical-align: middle;">Registration and welcome</td><td></td>
</tr>
  <td>09.00 - 10.30</td>
  <td><b>Time series and R, Time series graphics</b><br>
  <a href="#Lab1">Lab Sessions 1-2</a></td>
  <td style="vertical-align: middle;"><a href="??" target="_new">Slides</a></td>
</tr>
<tr>
  <td>10.30 - 11.00</td>
...
###Lab Session 8
  ...
  1. Use `ets` to find the best model for your retail data and record the training set RMSE.
  1. We will now check how much larger the one-step RMSE is on out-of-sample data using time series cross-validation. The following code will compute the
 result. Replace `???` with the appropriate values for your ETS model.

    ```{r, echo=TRUE, eval=FALSE}
    fets <- function(y, h)
    {
      fit <- ets(y, model=???)
      return(forecast(fit, h=1))
    }
    e <- tsCV(mytimeseries, fets, h=1)
    sqrt(mean(e^2, na.rm=TRUE))
    ```

  1. Plot `e` using `autoplot` and ``ggAcf``. Do they look uncorrelated and homoskedastic?

  1. In practice, we will not know the best model on the whole data set  until we observe all the data. So a more realistic analysis would be to allow `ets` to select a different model each time through the loop. Calculate the RMSE using this approach.  (Warning: there a lot of models to fit, so this will take a while.)

  1. How do the RMSE values compare? Does the re-selection of a model at each step make much difference?
