...
<a href="/nyc2018/2-2-STL.pdf" target="_new">Slides 2-2</a></td></tr>
<tr>
  <td>10.30 - 11.00</td>
  <td><em>Break</em></td><td></td>
</tr>
...
For this exercise, we will continue to use your retail data and use time series cross-validation to compare ETS and ARIMA models.
  ...
  1. First we will fit an ETS model.
     ...
     * We will now check how much larger the one-step RMSE is on out-of-sample data using time series cross-validation. The following code will compute the result, beginning with four years of data in the training set. Replace `???` with the appropriate values for your ETS model.
        ...
        etsfc <- function(y,h)
        {
          y %>% ets(model="???", damped=???) %>% forecast(h=h)
        }
  ...
  3. In practice, we will not know the best model on the whole data set  until we observe all the data. So a more realistic analysis would be to allow `ets()` and `auto.arima()` to select a different model each time through the loop. 
