...
This suggests that observation `r z$index` is an outlier, and an estimated replacement is `r round(z$replacement)`.


The `tsclean()` function removes outliers identified in this way, and replaces them (and any missing values) with estimates obtained using linear interpolation.

```{r}
plot(tsclean(gold), col='red')
lines(gold)
```
