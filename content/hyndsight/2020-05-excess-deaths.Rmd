...
deaths <- readr::read_csv("https://www.mortality.org/Public/STMF/Outputs/stmf.csv")
deaths
```
...
Clearly there are a few problems with the last week of available data from Finland, Norway and the United States. Most likely, these are simply incomplete counts. The effect of the pandemic results in tragic numbers for Belgium, England & Wales, the Netherlands, Spain, Sweden and the United States.
...
Before continuing, I'll remove that last week of data for Finland, Norway and the United States (which will make the lag one week longer for those countries)

```{r remove_lastweek, message=FALSE}
deaths <- deaths %>%
  group_by(country, year) %>%
  summarise(last_week = max(week), week=week, deaths=deaths) %>%
  ungroup() %>%
  filter(!(country %in% c("Finland", "Norway", "United States") & 
        year==2020 & week==last_week)) %>%
  select(year, week, country, deaths)
```
...
I'm fairly sure the numbers for the United States in the last week are also incorrect, as the official COVID19 deaths have not stopped decreasing, but the data will presumably get corrected over time.
